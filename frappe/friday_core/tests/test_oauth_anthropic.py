...
class TestBuildAuthorizeUrl(unittest.TestCase):
	@patch("frappe.friday_core.llm.oauth.anthropic.frappe")
...
class TestExchangeCode(unittest.TestCase):
	@patch("frappe.friday_core.llm.oauth.anthropic.now_datetime", return_value=_FIXED_NOW)
	@patch("frappe.friday_core.llm.oauth.anthropic.tokens")
	@patch("frappe.friday_core.llm.oauth.anthropic.requests")
	@patch("frappe.friday_core.llm.oauth.anthropic.frappe")
	...
	def test_state_mismatch_raises(self, mock_frappe, mock_tokens):
		from frappe.friday_core.llm.oauth import anthropic
		from frappe.friday_core.llm.oauth.tokens import OAuthError
