...
class TestStartClaudeLogin(unittest.TestCase):
	@patch(
		"frappe.friday_core.llm.oauth.anthropic.build_authorize_url",
		return_value="https://claude.ai/oauth/authorize?x=1",
	)
	@patch("frappe.friday_core.llm.oauth.endpoints.frappe")
	def test_flips_to_oauth_and_returns_url(self, mock_frappe, mock_build):
		from frappe.friday_core.llm.oauth import endpoints
...
class TestCompleteClaudeLogin(unittest.TestCase):
	@patch("frappe.friday_core.llm.oauth.endpoints.tokens")
	@patch("frappe.friday_core.llm.oauth.anthropic.exchange_code")
	@patch("frappe.friday_core.llm.oauth.endpoints.frappe")
...
class TestRefreshCron(unittest.TestCase):
	@patch("frappe.friday_core.llm.oauth_token_refresh.tokens")
	@patch("frappe.friday_core.llm.oauth_token_refresh.frappe")
	def test_refreshes_expiring_only(self, mock_frappe, mock_tokens):
		from frappe.friday_core.llm import oauth_token_refresh
