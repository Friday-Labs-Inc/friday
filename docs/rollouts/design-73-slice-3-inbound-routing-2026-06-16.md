...
**Status:** Built + verified live (Legion, real Northwind Coffee data).
...
- **The full Raven round-trip** (actual @mention in the UI → posted reply) wasn't run live — it needs a worker restart + an LLM call. The new *logic* is proven above, and the plumbing is pre-existing + unit-tested. Worth a smoke test on the next Legion deploy.
