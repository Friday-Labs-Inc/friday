...
def _resolve_parent(parameters: dict) -> tuple[str, str]:
	...
	if project_name:
		return "Agent Run", project_name
	if task_name:
		return "Agent Job", task_name
	raise ValueError(
...
def list_project_files(skill_name: str, parameters: dict) -> dict:
	...
	if not frappe.has_permission("Agent Run", "read", doc=project_name):
		return {
...
def get_project_file(skill_name: str, parameters: dict) -> dict:
	...
	if (
		getattr(file_doc, "attached_to_doctype", None) != "Agent Run"
