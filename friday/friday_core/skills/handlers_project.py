...
def project_status(skill_name: str, parameters: dict) -> dict:
	...
	if not frappe.db.exists("Agent Run", project):
		raise ValueError(f"Project {project!r} not found")

	status = frappe.db.get_value("Agent Run", project, "status")
	...
	return {"result": "\n".join(lines), "doctype": "Agent Run", "record_name": project}
...
def update_task(skill_name: str, parameters: dict) -> dict:
	...
	if not frappe.db.exists("Agent Job", task_name):
...
def pause_project(skill_name: str, parameters: dict) -> dict:
	...
	frappe.db.set_value("Agent Run", project, "status", new_status, update_modified=False)
	return {
