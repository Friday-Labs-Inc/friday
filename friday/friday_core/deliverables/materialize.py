...
def on_project_doc_update(doc, method=None) -> None:
	...
	if doc.get("status") == "Completed":
		frappe.enqueue(
			"friday.friday_core.deliverables.materialize.on_project_completed",
			queue="short",
...
def materialize_task_deliverable(task_name: str) -> "dict | None":
	...
	_clear_prior(task_name, "Agent Job")
	created = {"md": _attach(f"{_PREFIX}{slug}.md", body.encode("utf-8"), "Agent Job", task_name)}
	pdf = _render_pdf(title, content)
	if pdf:
		created["pdf"] = _attach(f"{_PREFIX}{slug}.pdf", pdf, "Agent Job", task_name)
	return created
...
def assemble_project_package(project_name: str) -> "dict | None":
	...
	tasks = frappe.get_all(
		"Agent Job", filters={"project": project_name}, fields=["name", "title"], order_by="creation asc"
	)
	sections = []
	for t in tasks:
		content = _extract_markdown(frappe.db.get_value("Agent Job", t["name"], "result"))
		if content:
			sections.append(f"## {t['title'] or t['name']}\n\n{content}")
	if not sections:
		return None

	project_title = frappe.db.get_value("Agent Run", project_name, "project_name") or project_name
	body = f"# {project_title} — Deliverable Package\n\n" + "\n\n---\n\n".join(sections)
	slug = _slug(project_name)

	_clear_prior(project_name, "Agent Run")
	created = {"md": _attach(f"{_PREFIX}{slug}.md", body.encode("utf-8"), "Agent Run", project_name)}
	pdf = _render_pdf(f"{project_title} — Deliverable Package", body)
	if pdf:
		created["pdf"] = _attach(f"{_PREFIX}{slug}.pdf", pdf, "Agent Run", project_name)
	return created
...
def materialize_for_customer(work_item_doctype: str, work_item_name: str) -> "dict | None":
	...
	files = frappe.get_all(
		"File",
		filters={"attached_to_doctype": "Agent Run", "attached_to_name": project},
	...
	for human_title, row in sources:
		...
		for prior in frappe.get_all(
			"File",
			filters={"attached_to_doctype": "Agent Run", "attached_to_name": project, "file_name": out_name},
			pluck="name",
		):
			try:
				...
				pass
		file_id = _attach(out_name, payload, "Agent Run", project)
