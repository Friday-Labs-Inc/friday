...
Usage:
    from friday.friday_core.sandbox import execute, SandboxResult
...
import frappe
from friday.friday_core.sandbox.credentials import generate_scoped_token
...
def _get_egress_config(agent_profile: str) -> tuple[str, list[str]]:
	...
	try:
		...
		if raw:
			...
			extra_hosts = [h.strip() for h in raw.replace("\n", ",").split(",") if h.strip()]
	except Exception:
		pass
...
def _resolve_limits(agent_profile: str, skill_name: str) -> tuple[int, int, int]:
	"""
...
def execute(
	...
	cpu_cores, memory_mb, resolved_timeout = _resolve_limits(agent_profile, skill_name)
		...
		for container in client.containers.list(all=True, filters={"label": "friday=true"}):
			...
			if isinstance(created_at, str):
				try:
					...
					created_ts = _time.mktime(_time.strptime(created_at[:19], "%Y-%m-%dT%H:%M:%S"))
				except Exception:
					continue
			else:
				continue
