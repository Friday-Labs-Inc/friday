...
keep everything inside friday/friday_core/" — see project memory), our
...
from friday.friday_core.cli.chat import run_repl
...
def setup(
	...
	import frappe
	from friday.friday_core.cli.setup import DEFAULT_MODELS, run_setup
...
def setup_raven(context, profile, install):
	...
	for site in context.sites:
		...
		try:
			...
			from friday.friday_core.surfaces.bootstrap_raven import provision
...
def worker(context, concurrency, quiet):
	...
	"""
	import frappe
	from friday.friday_core.agent_runner.worker_pool import resolve_concurrency, start_friday_worker
