...
from friday.friday_core.agent_runner.journal import EVENT_REPLY_DELIVERED, TurnJournal
from friday.friday_core.agent_runner.runner import run_turn
from friday.friday_core.gateway.batching import flush_batch
from friday.friday_core.permissions.matrix import check as permission_check
...
def _enqueue_pipeline(row_name: str, lock_retry: int) -> None:
	...
	frappe.enqueue(
		"friday.friday_core.gateway.service.run_pipeline_for_row",
		row_name=row_name,
...
def _run_pipeline(inbound, in_worker: bool = False, lock_retry: int = 0) -> None:
	...
	try:
		...
		from friday.friday_core.agent_runner.self_review import enqueue_if_due
