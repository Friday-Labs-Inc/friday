...
# the Link validation on LLM Usage Log / Agent Job made emit() swallow the row —
...
def _ensure_test_task() -> str:
	existing = frappe.get_all("Agent Job", limit=1, pluck="name")
	if existing:
		return existing[0]
	project = frappe.get_all("Agent Run", limit=1, pluck="name")
	project_name = project[0] if project else None
	if not project_name:
		proj = frappe.get_doc(
			{"doctype": "Agent Run", "project_name": "Test Write Sites Project"}
		).insert(ignore_permissions=True)
...
class TestWorkflowEmits(unittest.TestCase):
	...
	def test_state_change_emits_with_trigger_source(self):
		...
		task = frappe.get_doc("Agent Job", self.task_name)
	...
	def test_executing_token_released_event_fires(self):
		...
		frappe.db.sql(
			"UPDATE `tabAgent Job` SET workflow_state = 'Executing', "
...
class TestDispatcherSkipEmits(unittest.TestCase):
	...
	def test_milestone_skip_emits(self):
		...
		from friday.friday_core.tasks.dispatcher import _ready_to_dispatch
...
class TestLLMUsageEmit(unittest.TestCase):
	...
	def test_record_usage_emits_call_summary(self):
		from friday.friday_core.llm.usage import record_usage
...
class TestIssueRaisedEmit(unittest.TestCase):
	...
	def test_raise_failure_issue_emits_issue_raised(self):
		...
		frappe.db.commit()
		self.assertTrue(frappe.db.exists("Agent Blocker", issue_name))
		...
		frappe.db.sql("DELETE FROM `tabAgent Blocker` WHERE name = %s", (issue_name,))
		frappe.db.commit()
