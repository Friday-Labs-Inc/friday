...
class TestPoolConfig(unittest.TestCase):
	...
	def test_default_min_idle(self):
		from friday.friday_core.sandbox.pool import PoolConfig
...
class TestPoolStats(unittest.TestCase):
	...
	def test_returns_required_keys(self):
		from friday.friday_core.sandbox.pool import pool_stats
...
class TestAcquireOnEmptyPool(unittest.TestCase):
	...
	def test_empty_pool_returns_none(self):
		from friday.friday_core.sandbox.pool import acquire

		with patch("friday.friday_core.sandbox.pool._default_pool") as mock_defpool:
...
class TestReleaseErrorDestroys(unittest.TestCase):
	...
	def test_error_destroys(self):
		from friday.friday_core.sandbox.pool import release

		with patch("friday.friday_core.sandbox.pool._destroy_container") as mock_destroy:
	...
	def test_error_does_not_repool(self):
		...
		with patch("friday.friday_core.sandbox.pool._repool_container") as mock_repool:
			release("container-xyz", is_error=True)
	...
	def test_error_increments_destroyed_counter(self):
		...
		with patch("friday.friday_core.sandbox.pool._default_pool") as mock_defpool:
			with patch("friday.friday_core.sandbox.pool._destroy_container"):
				fake_state = _FakePoolState()
				mock_defpool.return_value = fake_state
...
class TestReleaseSuccessReturnsToPool(unittest.TestCase):
	...
	def test_success_calls_repool(self):
		from friday.friday_core.sandbox.pool import PoolConfig, release
	...
	def test_success_increments_returned_counter(self):
		...
		with patch("friday.friday_core.sandbox.pool._default_pool") as mock_defpool:
			with patch("friday.friday_core.sandbox.pool._repool_container"):
				fake_state = _FakePoolState()
				mock_defpool.return_value = fake_state
...
class TestDestroyContainer(unittest.TestCase):
	...
	def test_calls_all_cleanup_methods(self):
		from friday.friday_core.sandbox.pool import _destroy_container

		mock_container = MagicMock()
		with patch("friday.friday_core.sandbox.pool._get_client") as mock_client:
...
class TestRepoolContainer(unittest.TestCase):
	...
	def test_exec_run_then_pause(self):
		from friday.friday_core.sandbox.pool import _repool_container
...
class TestSpawnContainerSuccess(unittest.TestCase):
	...
	def test_returns_short_id_on_success(self):
		from friday.friday_core.sandbox.pool import _spawn_container
		...
		self.skipTest("importlib.reload defeats patch context; test design needs rework")
		with patch("friday.friday_core.sandbox.pool._get_client", return_value=mock_client):
			with patch("friday.friday_core.sandbox.pool.DOCKER_IMAGE", "test/image"):
				# Re-import to pick up patched constants
				import importlib

				import friday.friday_core.sandbox.pool as pool_mod
...
class TestSpawnContainerImageError(unittest.TestCase):
	...
	def test_returns_none_on_error(self):
		...
		with patch("friday.friday_core.sandbox.pool._get_client") as mock_client:
			...
			mock_client.return_value.images.pull.side_effect = Exception("pull failed")
...
class TestResizePoolSpawnsUpToMin(unittest.TestCase):
	...
	def test_spawns_missing_to_min_idle(self):
		from friday.friday_core.sandbox.pool import PoolConfig, _resize_pool
		...
		with patch("friday.friday_core.sandbox.pool._default_pool") as mock_defpool:
			...
			with patch("friday.friday_core.sandbox.pool._spawn_container") as mock_spawn:
...
class TestStopPool(unittest.TestCase):
	...
	def test_destroys_all_idle(self):
		from friday.friday_core.sandbox.pool import stop_pool
