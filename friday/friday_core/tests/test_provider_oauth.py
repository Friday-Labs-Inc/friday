...
class TestAnthropicAuthHeaders(unittest.TestCase):
	def _capture_headers(self, **provider_kwargs):
		from friday.friday_core.llm.provider import AnthropicProvider
...
class TestBuildProviderOAuthBranch(unittest.TestCase):
	@patch("friday.friday_core.llm.oauth.tokens.get_fresh_access_token", return_value="fresh-tok")
	def test_anthropic_oauth_builds_provider_with_token(self, _tok):
		from friday.friday_core.llm.provider import AnthropicProvider, _build_provider
	...
	def test_codex_oauth_builds_codex_provider(self, _tok):
		from friday.friday_core.llm.provider import CodexProvider, _build_provider
...
class TestCodexSSE(unittest.TestCase):
	...
	def test_parse_codex_sse_returns_completed_response(self):
		from friday.friday_core.llm.provider import _parse_codex_sse
	...
	def test_parse_codex_sse_without_completed_raises(self):
		from friday.friday_core.llm.provider import LLMError, _parse_codex_sse
