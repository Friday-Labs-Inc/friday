...
from friday.friday_core.llm import memory as M
...
class TestMemoryTypeField(unittest.TestCase):
	...
	def test_skill_schema_has_memory_type(self):
		from friday.friday_core.skills.bootstrap_memory import _SKILL
...
class TestRememberStoresType(unittest.TestCase):
	def _remember(self, params):
		...
		with (
			...
			patch("friday.friday_core.llm.memory.project_for_session", return_value=None),
			patch("friday.friday_core.llm.embed.enqueue_embed"),
		):
