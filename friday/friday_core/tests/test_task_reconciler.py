...
class TestReconcilerAssignedOrphans(unittest.TestCase):
	...
	@patch("friday.friday_core.tasks.reconciler.get_jobs")
	@patch("friday.friday_core.tasks.reconciler.frappe")
	def test_reenqueues_assigned_task_with_no_inflight_job(self, mock_frappe, mock_get_jobs):
		from friday.friday_core.tasks.reconciler import _reconcile_assigned_orphans
		...
		self.assertEqual(call.args[0], "friday.friday_core.tasks.runner.on_agent_task_assigned")
...
class TestReconcilerExecutingStale(unittest.TestCase):
	...
	@patch("friday.friday_core.tasks.reconciler._raise_runner_lost_issue")
	@patch("friday.friday_core.tasks.reconciler.get_jobs")
	@patch("friday.friday_core.tasks.reconciler.frappe")
	def test_blocks_executing_task_with_stale_heartbeat(self, mock_frappe, mock_get_jobs, mock_issue):
		from friday.friday_core.tasks.reconciler import _reconcile_executing_stale
...
class TestReconcilerTransientBlockedRetry(unittest.TestCase):
	...
	def test_retries_transient_blocked_under_budget(self, mock_frappe):
		from friday.friday_core.tasks.reconciler import _reconcile_transient_blocked
...
class TestReconcilerRandomPackEvents(unittest.TestCase):
	...
	def test_received_event_over_60s_old_reenqueues(self, mock_frappe):
		from friday.friday_core.tasks.reconciler import _reconcile_connector_events
		...
		mock_frappe.enqueue.assert_called_once()
		call = mock_frappe.enqueue.call_args
		self.assertEqual(call.args[0], "friday.friday_core.connectors.core.process_event")
		self.assertEqual(call.kwargs["queue"], "friday")
...
class TestTickWiring(unittest.TestCase):
	...
	@patch("friday.friday_core.tasks.reconciler._reconcile_connector_events")
	@patch("friday.friday_core.tasks.reconciler._reconcile_transient_blocked")
	@patch("friday.friday_core.tasks.reconciler._reconcile_executing_stale")
	@patch("friday.friday_core.tasks.reconciler._reconcile_assigned_orphans")
	def test_tick_runs_all_phases(self, mock_a, mock_e, mock_b, mock_r):
		from friday.friday_core.tasks.reconciler import tick
