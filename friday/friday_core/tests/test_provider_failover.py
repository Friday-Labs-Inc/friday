...
class TestFallbackResolution(unittest.TestCase):
	...
	def test_follows_the_link(self):
		from friday.friday_core.llm.provider import get_fallback_provider

		primary = MagicMock()
		primary.fallback_provider_name = "Codex"
		backup = MagicMock()
		with patch(
			"friday.friday_core.llm.provider.get_provider_by_name", return_value=backup
		) as mg:
	...
	def test_broken_link_returns_none(self):
		...
		with patch(
			"friday.friday_core.llm.provider.get_provider_by_name",
