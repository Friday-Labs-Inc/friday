...
from friday.friday_core.llm.usage import estimate_cost, record_usage

_U = "friday.friday_core.llm.usage"
...
class TestRunnerWiring(unittest.TestCase):
	def test_runner_records_usage_once_per_chat_call(self):
		from friday.friday_core.tests.test_react_loop import _provider as _chat_provider
		from friday.friday_core.tests.test_react_loop import _resp, _run

		prov = _chat_provider(_resp(content="Hello!"))
		prov.get_default_model.return_value = "MiniMax-M2"
		with patch("friday.friday_core.agent_runner.runner.record_usage") as mock_record:
