...
class TestConsoleSnapshot(unittest.TestCase):
	@patch("friday.friday_core.console.console_snapshot.pipeline_health")
	@patch("friday.friday_core.console.console_snapshot.frappe")
	def test_returns_all_zones(self, mock_frappe, mock_health):
		from friday.friday_core.console.console_snapshot import console_snapshot
	...
	def test_active_tasks_query_filters_in_flight_states(self, mock_frappe, mock_health):
		from friday.friday_core.console.console_snapshot import ACTIVE_STATES, console_snapshot
		...
		state_filters = [
			...
			if c.args and c.args[0] == "Agent Job"
		]
