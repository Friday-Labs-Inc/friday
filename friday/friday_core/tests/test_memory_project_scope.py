...
def _project(name: str) -> str:
	if not frappe.db.exists("Agent Run", name):
		frappe.get_doc(
			{"doctype": "Agent Run", "project_name": name, "status": "Open"}
		).insert(ignore_permissions=True)
	return name
...
class TestProjectForSession(unittest.TestCase):
	def test_task_session_resolves_project(self):
		...
		task = frappe.get_doc(
			{"doctype": "Agent Job", "title": "mem-scope task", "project": proj, "workflow_state": "Pending"}
		).insert(ignore_permissions=True)
		try:
			self.assertEqual(project_for_session(f"task::{task.name}"), proj)
		finally:
			frappe.delete_doc("Agent Job", task.name, force=True, ignore_permissions=True)
...
class TestRememberTagsProject(unittest.TestCase):
	def test_remember_tags_session_project(self):
		...
		task = frappe.get_doc(
			{"doctype": "Agent Job", "title": "remember-scope task", "project": proj, "workflow_state": "Pending"}
		).insert(ignore_permissions=True)
