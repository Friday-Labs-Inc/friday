...
def _project() -> str:
	existing = frappe.get_all("Agent Run", limit=1, pluck="name")
	if existing:
		return existing[0]
	return frappe.get_doc(
		{"doctype": "Agent Run", "project_name": "Deliverables Test Project", "status": "Open"}
...
def _cleanup_task(task_name: str) -> None:
	for f in frappe.get_all("File", filters={"attached_to_doctype": "Agent Job", "attached_to_name": task_name}, pluck="name"):
		frappe.delete_doc("File", f, force=True, ignore_permissions=True)
	if frappe.db.exists("Agent Job", task_name):
		frappe.delete_doc("Agent Job", task_name, force=True, ignore_permissions=True)
	frappe.db.commit()
...
class TestMaterializeTask(unittest.TestCase):
	...
	def test_creates_markdown_artifact(self):
		...
		files = _deliverable_files("Agent Job", self.task)
	...
	def test_idempotent_replaces_prior_files(self):
		...
		frappe.db.commit()
		first = set(_deliverable_files("Agent Job", self.task))
		materialize_task_deliverable(self.task)
		frappe.db.commit()
		# Same logical files, not doubled.
		count_md = sum(1 for f in _deliverable_files("Agent Job", self.task) if f.endswith(".md"))
...
class TestNoArtifactForNonDeliverable(unittest.TestCase):
	...
	def test_gate_with_empty_result_no_artifact(self):
		...
		try:
			...
			self.assertEqual(_deliverable_files("Agent Job", task), [])
		finally:
			_cleanup_task(task)
...
class TestProjectPackage(unittest.TestCase):
	...
	def test_package_combines_task_deliverables(self):
		...
		project = frappe.get_doc(
			{"doctype": "Agent Run", "project_name": "Pkg Test Project", "status": "Open"}
		).insert(ignore_permissions=True).name
		...
		try:
			...
			_cleanup_task(t1)
			_cleanup_task(t2)
			for f in frappe.get_all("File", filters={"attached_to_doctype": "Agent Run", "attached_to_name": project}, pluck="name"):
				frappe.delete_doc("File", f, force=True, ignore_permissions=True)
			frappe.delete_doc("Agent Run", project, force=True, ignore_permissions=True)
			frappe.db.commit()
