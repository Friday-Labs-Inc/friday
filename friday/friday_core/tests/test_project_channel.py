...
def _cleanup_project(project_name: str) -> None:
	...
	ch = frappe.db.get_value("Agent Run", project_name, "conversation_channel") if frappe.db.exists(
	...
	if frappe.db.exists("Agent Run", project_name):
		frappe.delete_doc("Agent Run", project_name, force=True, ignore_permissions=True)
	frappe.db.commit()
...
class TestProjectChannelProvision(unittest.TestCase):
	...
	def test_creating_project_provisions_linked_channel(self):
		...
		channel = frappe.db.get_value("Agent Run", project, "conversation_channel")
		...
		self.assertEqual(linked["linked_doctype"], "Agent Run")
	...
	def test_provision_is_idempotent(self):
		from friday.friday_core.conversation.project_channel import provision_project_channel

		project = _make_project(self.pname, backend_ref="D73-ALPHA")
		frappe.db.commit()
		first = frappe.db.get_value("Agent Run", project, "conversation_channel")
...
class TestProjectChannelArchive(unittest.TestCase):
	...
	def test_completing_project_archives_channel(self):
		...
		doc = frappe.get_doc("Agent Run", project)
...
class TestProjectChannelGraceful(unittest.TestCase):
	...
	def test_provision_failure_does_not_break_project_insert(self):
		...
		try:
			with patch(
				"friday.friday_core.conversation.project_channel.provision_project_channel",
				...
				frappe.db.commit()
				self.assertTrue(frappe.db.exists("Agent Run", project))
		finally:
			_cleanup_project(pname)
