...
from friday.friday_core.agent_runner import self_review as SR

_S = "friday.friday_core.agent_runner.self_review"
_SETTINGS = "friday.friday_core.doctype.agent_settings.agent_settings.get_agent_settings"
...
class TestRunReview(unittest.TestCase):
	_RT = "friday.friday_core.agent_runner.runner.run_turn"
...
class TestResolveReviewProvider(unittest.TestCase):
	...
	def test_named_resolves_provider(self):
		...
		with patch(_SETTINGS, return_value=doc), patch(
			"friday.friday_core.llm.provider.get_provider_by_name", return_value=prov
		) as gp:
