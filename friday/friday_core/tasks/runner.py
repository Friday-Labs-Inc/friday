...
def _get_warroom():
	...
	if _warroom is None:
		try:
			from friday.friday_core import warroom
...
def _claim_task(task_name: str) -> str | None:
	...
	current = frappe.db.get_value("Agent Job", task_name, "executing_token")
...
def _heartbeat(task_name: str) -> None:
	...
	"""
	try:
		frappe.db.set_value("Agent Job", task_name, "last_heartbeat_at", now_datetime(), update_modified=False)
	except Exception:
...
def _execute_skill_in_sandbox(skill_name: str, task: "AgentJob", profile_name: str) -> "SandboxResult":
	"""
	...
	"""
	from friday.friday_core.sandbox import credentials as sandbox_creds
	from friday.friday_core.sandbox import runner as sandbox_runner
...
def _parse_task_parameters(task: "AgentJob", skill_name: str) -> dict:
	"""
...
def _block_task(task: "AgentJob", results: list) -> None:
	"""
...
def _run_task_agentic(task: "AgentJob", profile_name: str) -> None:
	...
	try:
		...
		if frappe.db.get_value("Agent Job", task.name, "force_killed_by"):
			return
...
def _task_transition(task: "AgentJob", target_state: str) -> None:
	"""
