...
def _watch_transition(doc: "Task") -> None:
	...
	from friday.friday_core.tasks.report_back import report_back
	...
	if state in ("Completed", "Blocked", "Cancelled"):
		try:
			...
			from friday.friday_core.cron.scheduler import on_task_terminal
	...
	if state == "Completed":
		frappe.enqueue(
			"friday.friday_core.deliverables.materialize.on_task_completed",
			queue="short",
	...
	from friday.friday_core.observability import is_terminal, write_task_completion_summary
