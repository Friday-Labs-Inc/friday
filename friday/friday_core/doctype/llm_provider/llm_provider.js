...
frappe.ui.form.on("LLM Provider", {
	refresh: function (frm) {
		...
		if (frm.doc.auth_mode === "oauth") {
			frappe.call({
				method: "friday.friday_core.llm.oauth.endpoints.oauth_status",
				args: { provider_name: frm.doc.name },
				callback: function (r) {
					const s = r && r.message;
					if (!s) return;
...
function start_claude_login(frm) {
	frappe.call({
		method: "friday.friday_core.llm.oauth.endpoints.start_claude_login",
		args: { provider_name: frm.doc.name },
		freeze: true,
		...
		callback: function (r) {
			...
			const d = new frappe.ui.Dialog({
				...
				primary_action: function (values) {
					frappe.call({
						method: "friday.friday_core.llm.oauth.endpoints.complete_claude_login",
...
function start_codex_login(frm) {
	frappe.call({
		method: "friday.friday_core.llm.oauth.endpoints.start_codex_login",
		args: { provider_name: frm.doc.name },
		freeze: true,
		...
		callback: function (r) {
			...
			const poll = () => {
				frappe.call({
					method: "friday.friday_core.llm.oauth.endpoints.poll_codex_login",
					args: { provider_name: frm.doc.name },
					callback: function (res) {
						const s = res && res.message;
						if (s && s.logged_in) {
