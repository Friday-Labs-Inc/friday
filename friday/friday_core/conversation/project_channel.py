...
# ---------------------------------------------------------------------------
# doc_events hooks (registered in hooks.py for "Agent Run")
# ---------------------------------------------------------------------------
...
def provision_project_channel(project_name: str) -> "str | None":
	...
	project = frappe.get_doc("Agent Run", project_name)
	...
	frappe.db.set_value(
		"Agent Run", project.name, "conversation_channel", channel.name, update_modified=False
	)
...
def post_seed_welcome(channel_id: str, project_name: str) -> None:
	...
	if not frappe.db.exists("Raven Channel", channel_id):
		return
	if not frappe.db.exists("Agent Run", project_name):
		return
	project = frappe.get_doc("Agent Run", project_name)
...
def archive_project_channel(project_name: str) -> None:
	...
	channel = frappe.db.get_value("Agent Run", project_name, "conversation_channel")
